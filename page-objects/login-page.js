...
const { expect } = require("chai");
 ...
 class LoginPage {
  ...
  async testy() {
    ...
    const present = await page.getByRole("presentation");
    const przycisk = await page.getByRole("button");
    const box = await present.boundingBox();

    console.log("getbyRole " + await box.height);
    console.log("przycisk " + await przycisk.count());
    console.log("all " + await przycisk.allTextContents());

    for (const li of await przycisk.all())
      console.log("iteracja " + await li.textContent());
  }
